...
from file_query.api.context.df_base import ExecutionContext
from file_query.api.core.types import Engines


def _duckdb(chunk_size: int):
    from file_query.api.context.df_duckdb import DuckDbExecutionContext

    return DuckDbExecutionContext(chunk_size=chunk_size)


def _polars(chunk_size: int):
    from file_query.api.context.df_polars import PolarsExecutionContext

    return PolarsExecutionContext(chunk_size=chunk_size)


def _odbc(chunk_size: int):
    from file_query.api.context.df_odbc import ODBCExecutionContext
